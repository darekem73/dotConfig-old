Window() {
	...
	echo "^fg(#286d74)^i($icon)^fg() $title"
}

Mem() {
	icon="$ICONS/Mem.xbm"
	command=$(free -h | awk '/Mem/{print $3, $2}' OFS=' / ')
	echo "^fg(#e5d693)^i($icon)^fg() $command"
}

Temp() {
	icon="$ICONS/Temp.xbm"
	command=$(sensors | awk '/Core/{printf("%i°C\n", $3)}')
	echo "^fg(#d21c04)^i($icon)^fg() $command"
}

Pacotes() {
	icon="$ICONS/Packages.xbm"
	command=$(ls -d /var/db/pkg/*/* | wc -l)
	echo "^ca(1,~/scripts/Shells/sysinfo.sh)^fg(#e6b60a)^i($icon)^fg() $command^ca()"
}
...
Musica() {
	...
	echo "^ca(1,~/scripts/Shells/mpd.sh)^ca(4,mpc volume +10)^ca(5,mpc volume -10)^fg(#286d74)^i($icon)^fg() $command^ca()^ca()^ca()"
}

Volume() {
	icon="$ICONS/Volume.xbm"
	command=$(amixer get Master | awk '/[0-9]%/{gsub(/[][]/,"");print $5}' | head -1 | gdbar -h "3" -bg "#505050" -fg "#e5d693" -w "70")
	echo "^ca(4,amixer set Master 5%+)^ca(5,amixer set Master 5%-)^fg(#e5d693)^i($icon)^fg() $command^ca()^ca()"
}
...
Hora() {
	...
	echo "^ca(1,~/scripts/Shells/calendario.sh)^fg(#e6b60a)^i($icon)^fg() $command^ca()"
}
...
dzen2 -p -bg "#101010" -h "30" -e "button3=" &
...
while true;do
    echo "$(Works)"
    sleep 2
done | dzen2 -p -ta "c" -bg "#101010" -fg "#777777" -fn "FantasqueSansMono-9" -h "14" -e "button3=" -y "7" &
...
while true;do
	...
	sleep 2
done | dzen2 -p -ta "l" -bg "#101010" -fg "#777777" -fn "FantasqueSansMono-9" -h "30" -e "button3=" -w "535" &
...
while true;do
	...
	sleep 2
done | dzen2 -p -ta "r" -bg "#101010" -fg "#777777" -fn "FantasqueSansMono-9" -h "30" -e "button3=" -x "900" &
